...
function Transformer(logger) {
     ...
     * var Transformer = require('jy-transform');
     * var transformer = new Transformer(logger);
     * transformer.itmo(options, reader.readYaml, transformer.ensureMiddleware(middleware), writer.writeJson);
     * @private
     */
     ...
     *        function(json)
     *        ```
     *        **NOTE:** the Promise has to return the processed JSON object!
     * @example
     * var logger = ...;
     * var reader = new Reader(logger);
     * var writer = new Writer(logger);
