 ...
 * @param logger {logger|cli|console} (optional) default is <tt>console</tt>
...
var Transformer = function (options, logger) {
    ...
    this.debug('invalidTarget: ' + (constants.TARGETS.indexOf(options.target) < 0));

    var invalidTarget = function () {
        return (constants.TARGETS.indexOf(options.target) < 0);
    };

    if (invalidTarget === true) {
        fatal('Invalid target \'' + options.target + '\' found, must be one of ' + JSON.stringify(constants.TARGETS));
    }
