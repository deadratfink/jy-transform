        ...
        .then(function (assertedOptions) {
            return new Promise(function (resolve, reject) {
                ...
                if (Constants.TRANSFORMATIONS.indexOf(transformation) < 0) {
                    return reject(new Error('Unsupported target type transformation \'' + assertedOptions.origin + ' -> ' + assertedOptions.target + '\' configured in options.'));
                } else {
                    return resolve([assertedOptions, transformation]);
                }
            });
    });
};
