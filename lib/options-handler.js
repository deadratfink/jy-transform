    ...
    return new Promise(function (resolve, reject) {
      if (typeof options.src === 'string') {
        ...
        try {
          ...
          // here we get a "native" error which seems to be not type of Error but simple Object!
          // For the sake of consistency we turn it into real Error type.
          const realError = new Error('Error occurred while checking input file \'' + options.src + '\' which should be existing and accessible, code: ' + err.code + ', cause: ' + err.message);
          realError.code = err.code;
          realError.errno = err.errno;
          realError.syscall = err.syscall;
          realError.path = err.path;
