...
function OptionsHandler(logger) {
        ...
        return assertOptions(options, ['src'])
            .then(function (assertedOptions) {
                ...
                if (assertedOptions.dest && (typeof assertedOptions.dest === 'string')) {
                    ...
                    destType = getTypeFromFilePath(assertedOptions.dest, false, Constants.JS); // TODO: what about Buffer/stream?
                }
    ...
    this.ensureSrc = function (options) {
        ...
        return assertOptions(options, ['src'])
            ...
            .spread(function (checked, options) {
                if (!checked) {
                    return assertBufferSrc(options);
                }
                return [checked, options];
            })
            .spread(function (checked, options) {
    ...
    function assertBufferSrc(options) {
        return new Promise(function (resolve, reject) {
            if (options.src instanceof Buffer) {
                self.logInstance.debug('options.src is Buffer');
                if (!options.origin) {
                    return reject(new Error('when options.src is a Buffer, then setting options.origin is mandatory!'));
                } else {
                    return resolve([true, options]);
                }
            } else {
