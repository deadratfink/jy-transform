...
function OptionsHandler(logger) {
    ...
    this.completeOptions = function (options) {
        return new Promise(function (resolve, reject) {
            if (options) {
                ...
                options.origin = (options.origin && (options.origin !== Constants.ORIGIN_DESCRIPTION)) ? options.origin : (srcType || Constants.DEFAULT_ORIGIN);
                options.target = (options.target && (options.target !== Constants.TARGET_DESCRIPTION)) ? options.target : (destType || Constants.DEFAULT_TARGET);
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (typeof options.src === 'string') {
                ...
                try {
                    ...
                    err.message = 'Error occurred while checking input file \'' + options.src + '\' which should be exising and accessible, code: ' + err.code + ', cause: ' + err.message;
                    return reject(err);
                }
    ...
    this.ensureIndent = function (options) {
        self.logInstance.debug('options before ensureIndent():: ' + JSON.stringify(options, null, 4));
        if (options.indent === undefined) {
            ...
            self.logInstance.info('Invalid indent \'' + options.indent + '\' found for JS/JSON, reset to default: ' + Constants.DEFAULT_INDENT);
            options.indent = Constants.DEFAULT_INDENT;
        ...
        self.logInstance.debug('options after ensureIndent():: ' + JSON.stringify(options, null, 4));
        return Promise.resolve(options);
    };
