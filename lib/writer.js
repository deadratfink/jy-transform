...
function Writer(logger) {
    ...
    function serializeJsToString(json, indent) {
        // For feature #4 : (this.options.prefix ? 'module.exports = ' : '')
            ...
            return mkdirp(destDir)
                .then(function () {
                    ...
                    if (forceOverwrite === undefined || forceOverwrite === false) {
                        ...
                        self.logInstance.info('Setting was: do not overwrite,...so new destination ' + dest + ' created');
                    }
        ...
        return self.optionsHandler.ensureIndent(options)
            .then(function (ensuredOptions) {
                return new Promise(function (resolve, reject) {
                    ...
                    var indent = ensuredOptions.indent;
                    if (typeof dest === 'string') { // file
                        return writeToFile(serializeJsToString(json, indent), dest, Constants.JS, resolve, reject, ensuredOptions.force);
                    } else if (isStream.writable(dest)) { // stream
